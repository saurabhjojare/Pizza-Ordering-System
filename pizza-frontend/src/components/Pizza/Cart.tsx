...
const Cart: React.FC<CartProps> = ({ cartItems, removeFromCart, updateQuantity, clearCart }) => {
  ...
  const calculateTotalAmount = () =>
    cartItems.reduce((total, item) => {
      ...
      return total + ((pizzaPrice + item.crust.price + toppingsPrice) * item.quantity);
    }, 0);

  const totalAmount = calculateTotalAmount();

  const handleQuantityChange = (index: number, increment: boolean) => {
    const newQuantity = increment ? cartItems[index].quantity + 1 : cartItems[index].quantity - 1;
    if (newQuantity > 0) {
          ...
          {cartItems.map((item, index) => {
            ...
            return (
              <li key={index} className="list-group-item d-flex justify-content-between align-items-center">
                <div className='fs-5'>
                  <p className="mb-1"><strong>Pizza:</strong> {item.pizza.name}</p>
                  <p className="mb-1"><strong>Size:</strong> {item.size}</p>
                  <p className="mb-1"><strong>Crust:</strong> {item.crust.name} : ₹{item.crust.price}</p>
                  <p className="mb-1">
                  ...
                  <p className="mb-1"><strong>Toppings: </strong> 
                    {item.selectedToppings.map(topping => (
                      <span key={topping.name}>{topping.name} : ₹{topping.price} </span>
                    ))}
                  </p>
                </div>
